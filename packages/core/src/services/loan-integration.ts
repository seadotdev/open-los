...
import type { LoanAccountService, CreateLoanAccountInput } from "./loan-account.js";
 ...
 */
function mapInterestRateType(
  facilityRateType: string | null
): "FIXED" | "FLOATING" | undefined {
  if (!facilityRateType) return undefined;
 ...
 */
function mapRepaymentMethod(
  facilityType: string
): "EQUAL_INSTALLMENTS" | "BALLOON" | "INTEREST_ONLY" {
  switch (facilityType) {
    case "revolver":
 ...
 */
function calculateFirstRepaymentDate(
...
export interface CreateLoanFromFacilityInput {
  facility_id: string;
  deal_id: string;
  account_holder_id: string; // entity ID of the borrower
 ...
 */
export async function createLoanAccountFromFacility(
  db: Database,
  loanAccountService: LoanAccountService,
  input: CreateLoanFromFacilityInput,
  actor: string,
  tenantId = "default"
) {
  ...
  if (!accountHolderId) {
    throw new NotFoundError(
      "No account holder specified and deal has no primary entity"
    );
  }
  ...
  const firstRepaymentDate =
    input.first_repayment_date ?? calculateFirstRepaymentDate(now, input.grace_period_days);

  // Build loan account input
  const loanInput: CreateLoanAccountInput = {
 ...
 */
export async function approveLoanAccount(
  loanAccountService: LoanAccountService,
 ...
 */
export async function createApprovedLoanFromFacility(
  db: Database,
  loanAccountService: LoanAccountService,
  input: CreateLoanFromFacilityInput,
  ...
  // Create the loan account
  const loanAccount = await createLoanAccountFromFacility(
    db,
    loanAccountService,
    input,
    actor,
 ...
 */
export async function getLoanBalanceForDeal(
  loanAccountService: LoanAccountService,
  dealId: string,
  tenantId = "default"
): Promise<{ total_outstanding: number; principal_outstanding: number }> {
 ...
 */
export async function checkDealArrearsStatus(
  loanAccountService: LoanAccountService,
  dealId: string,
  asOfDate: string,
  tenantId = "default"
