...
export interface UpdateEntityInput {
  ...
  lei?: string;
  jurisdiction?: string;
  type?: string; // For validation - cannot be changed
}
...
export class EntityService {
  ...
  async update(id: string, input: UpdateEntityInput, actor: string, dealId?: string, tenantId = "default") {
  ...
  async list(tenantId = "default", filters?: { type?: string; limit?: number; cursor?: string }) {
    let rows;
    if (filters?.type) {
  ...
  async delete(id: string, tenantId = "default") {
    ...
    const relatedRows = await this.db
      ...
      .where(
        or(
          eq(relationships.from_entity_id, id),
          eq(relationships.to_entity_id, id)
        )
      );
  ...
  async findDealIdByEntity(entityId: string): Promise<string | null> {
