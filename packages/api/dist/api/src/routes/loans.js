...
import { createLoanAccountFromFacility, createApprovedLoanFromFacility, getLoanBalanceForDeal, checkDealArrearsStatus, } from "@open-los/core";
export function loanRoutes(ctx) {
    const app = new Hono();
    // ─── Loan Accounts ─────────────────────────────────────────────────────────
    ...
    app.post("/loans/:loanId/transactions", async (c) => {
        ...
        switch (body.type) {
            ...
            case "PENDING_APPROVAL":
                result = await ctx.loanAccountService.requestApproval(loanId, body.notes ?? "", actor, tenantId);
                break;
            case "APPROVAL":
                result = await ctx.loanAccountService.approve(loanId, body.notes ?? "", actor, tenantId);
                break;
            case "REJECT":
                result = await ctx.loanAccountService.reject(loanId, body.notes ?? "", actor, tenantId);
                break;
            case "WITHDRAW":
                result = await ctx.loanAccountService.withdraw(loanId, body.notes ?? "", actor, tenantId);
                break;
            case "LOCK":
                result = await ctx.loanAccountService.lock(loanId, body.notes ?? "", actor, tenantId);
                break;
            case "UNLOCK":
                result = await ctx.loanAccountService.unlock(loanId, body.notes ?? "", actor, tenantId);
                break;
            case "CLOSE":
                result = await ctx.loanAccountService.close(loanId, body.subState, body.notes ?? "", actor, tenantId);
                break;
            // Financial operations
            case "DISBURSEMENT":
                result = await ctx.loanAccountService.disburse(loanId, {
                    amount: body.amount,
                ...
                break;
            case "REPAYMENT":
                result = await ctx.loanAccountService.repay(loanId, {
                    amount: body.amount,
                    value_date: body.valueDate,
                    idempotency_key: idempotencyKey ?? body.idempotencyKey,
                ...
                break;
            case "FEE":
                result = await ctx.loanAccountService.applyFee(loanId, {
                    amount: body.amount,
                    value_date: body.valueDate,
                    idempotency_key: idempotencyKey ?? body.idempotencyKey,
                    notes: body.notes,
            ...
            case "INTEREST_APPLIED":
                result = await ctx.loanAccountService.applyInterest(loanId, body.valueDate ?? body.asOfDate, actor, tenantId);
                break;
            case "WRITE_OFF":
                result = await ctx.loanAccountService.writeOff(loanId, body.notes ?? "", actor, tenantId);
                break;
            default:
                return c.json({
                    error: {
    ...
    app.get("/loans/:loanId/transactions", async (c) => {
        ...
        const result = await ctx.loanAccountService.getTransactions(loanId, { limit, cursor }, tenantId);
        return c.json(stripNulls(result), 200);
    });
    ...
    app.get("/deals/:dealId/loans/balance", async (c) => {
        const dealId = c.req.param("dealId");
        const tenantId = c.req.header("X-Tenant-Id") ?? "default";
        const result = await getLoanBalanceForDeal(ctx.loanAccountService, dealId, tenantId);
        return c.json(stripNulls(result), 200);
    });
    ...
    app.get("/deals/:dealId/loans/arrears", async (c) => {
        ...
        const tenantId = c.req.header("X-Tenant-Id") ?? "default";
        const asOfDate = c.req.query("asOfDate") ?? new Date().toISOString().split("T")[0];
        const result = await checkDealArrearsStatus(ctx.loanAccountService, dealId, asOfDate, tenantId);
        return c.json(stripNulls(result), 200);
    });
    ...
    app.post("/facilities/:facilityId/loans", async (c) => {
        ...
        const tenantId = c.req.header("X-Tenant-Id") ?? "default";
        const body = await c.req.json();
        const result = await createLoanAccountFromFacility(ctx.db, ctx.loanAccountService, {
    ...
    app.post("/facilities/:facilityId/loans/approved", async (c) => {
        ...
        const tenantId = c.req.header("X-Tenant-Id") ?? "default";
        const body = await c.req.json();
        const result = await createApprovedLoanFromFacility(ctx.db, ctx.loanAccountService, {
            facility_id: facilityId,
            deal_id: body.dealId,
            account_holder_id: body.accountHolderId,
            first_repayment_date: body.firstRepaymentDate,
            ...
            grace_period_days: body.gracePeriodDays,
            custom_fields: body.customFields,
        }, body.approvalNotes ?? "Auto-approved from facility", actor, tenantId);
        return c.json(stripNulls(result), 201);
    });
    return app;
}
