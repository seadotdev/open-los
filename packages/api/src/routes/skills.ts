...
export function skillRoutes(ctx: AppContext) {
  const app = new Hono();

  // GET /v1/skills - List available skills (metadata only)
  app.get("/skills", async (c) => {
    const tenantId = c.req.header("X-Tenant-Id") ?? "default";
    const scope = c.req.query("scope") as "public" | "org" | "user" | undefined;
  ...
  app.post("/skills", async (c) => {
    ...
    if (!body.name || !body.description || !body.path) {
      return c.json(
        { error: { code: "VALIDATION_ERROR", message: "name, description, and path are required" } },
        400
      );
    }
  ...
  app.patch("/skills/:nameOrId", async (c) => {
    ...
    const skill = await ctx.skillService.update(
      nameOrId,
      {
        description: body.description,
        trigger: body.trigger,
  ...
  app.post("/skills/:nameOrId/invocations", async (c) => {
    ...
    const invocation = await ctx.skillService.startInvocation(
      nameOrId,
      {
        dealId: body.dealId,
  ...
  app.patch("/skills/invocations/:invocationId", async (c) => {
    ...
    if (!body.status || !["completed", "failed"].includes(body.status)) {
      return c.json(
        { error: { code: "VALIDATION_ERROR", message: "status must be 'completed' or 'failed'" } },
        400
      );
    }
